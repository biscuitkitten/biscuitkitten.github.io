...
Game.Launch=function()
{
	...
	Game.Init=function()
	{
		...
		Game.DefaultPrefs=function()
		{
			...
			Game.prefs.offprog=1;//do offline progress
			Game.prefs.newsticker=1;//show news
			Game.prefs.scinotation=0;//display numbers in scientific notation
		...
		Game.WriteSave=function(type)
		{
			...
			(Game.prefs.offprog?'1':'0')+
			(Game.prefs.newsticker?'1':'0')+
			(Game.prefs.scinotation?'1':'0')+
			'';
			str2=pack3(str2);
			str+=str2+'|';
			if (type==3) str+='\n\nMisc game data';
			...
			if (str!='')
			{
				...
				else
				{
					...
					if (version>=1)
					{
						...
						Game.prefs.offprog=parseInt(spl[27]);
						Game.prefs.newsticker=parseInt(spl[28]);
						Game.prefs.scinotation=parseInt(spl[29]);
						BeautifyAll();
						...
						if (Game.prefs.offprog && (Game.mobile || Game.Has('Offline Baker') || Game.Has('Perfect idling') || Game.Has('Twin Gates of Transcendence')))
						{
							...
							{
								var maxTime=60*60;
								if (Game.Has('Offline Baker')) maxTime = 60*60*24;
							...
							if (amount>0)
							{
								Game.Notify(loc("Welcome back!"),loc("You earned <b>%1</b> while you were away.",loc("%1 cookie",LBeautify(amount)))+(EN?('<br>('+Game.sayTime(timeOfflineOptimal*Game.fps,-1)+' at '+Math.floor(percent)+'% CpS'+(timeOfflineReduced?', plus '+Game.sayTime(timeOfflineReduced*Game.fps,-1)+' at '+(Math.floor(percent*10)/100)+'%':'')+'.)'):''),[36,0]);
								Game.Earn(amount * Game.cheaterBoost);
							}
						}
		...
		Game.computeLumpTimes=function()
		{
			...
			Game.lumpMatureAge /= Game.cheaterBoost; // lumps mature faster based on cheater boost
			Game.lumpRipeAge /= Game.cheaterBoost; // lumps ripen faster based on cheater boost
			Game.lumpOverripeAge /= Game.cheaterBoost; // lumps fall faster based on cheater boost
		}
		...
		Game.ClickCookie=function(e,amount,auto=false)
		{
		...
		Game.CalculateGains=function()
		{
			...
			if (Game.Has('Autoclick switch [off]')) { 
				...
				if (Game.Has('Autoclick v1.3')) Game.computedAutoCps += 0.5
				if (Game.Has('Autoclick v1.4')) Game.computedAutoCps += 0.5
		...
		Game.UpdateMenu=function()
		{
			...
			else if (Game.onMenu=='stats')
			{
				...
				'<div id="statsGeneral">'+
					...
					(Game.computedAutoCps!=0?'<div class="listing"><b>'+loc("Switch Autoclicks per second:")+'</b> '+Beautify(Game.computedAutoCps,2)+'</div>':'')+
		...
		Game.getNewTicker=function(manual)//note : "manual" is true if the ticker was clicked, but may also be true on startup etc
		{
			...
			if(!Game.prefs.newsticker) Game.Ticker=""
		...
		new Game.Upgrade('Autoclick switch [on]',"The switch is currently <b>autoclicking the cookie</b>; it also prevents you from clicking manually.<br>Turning it off will revert those effects.<br>Cost is equal to 1 minute of production.",1000,[37,36]);
		Game.last.pool='toggle';Game.last.toggleInto='Autoclick switch [off]';
		Game.last.priceFunc=function(){return Game.cookiesPs*60;}
				...
				{	
					if (Game.prefs.particles)
					{
						...
						else
						{
							...
							ctx.drawImage(Pic('perfectBiscuit.png'),-s/2,-s/2,s,s);
					...
					else//no particles
					{
						...
						ctx.drawImage(Pic('perfectBiscuit.png'),x,y,s,s);
					}
	...
	Game.Logic=function()
	{
		...
		if (!Game.OnAscend && Game.AscendTimer==0)
		{
			...
			if (Game.T%(Game.fps*5)==0 && Game.ObjectsById.length>0)//check some achievements and upgrades
			{
				...
				if (!Game.HasAchiev('Third-party')) Game.Win('Third-party') // immediately gained because... this is a mod
				
				// unlock offline progress at 10 cookies
...
// BISCUITCLICKER STUFF
let AcS_UpdRate = 20
var clickPow = 0
var lastTime = new Date().getTime();
var AutoclickSwitch = setInterval(function(){
  if (Game.computedAutoCps) {
	  try {
		let currTime = new Date().getTime();
		clickPow += Game.computedAutoCps*(currTime-lastTime)/1000
		lastTime = currTime
		if(clickPow > 1) {
			clickPow = 0
			Game.ClickCookie(undefined,undefined,auto=true)
		}
	  } catch (err) {
		console.error('Something went wrong with the autoclick switch! OH NOOO!');
	  }
  }
}, AcS_UpdRate);
